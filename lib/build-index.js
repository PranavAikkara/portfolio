...
export function parseMarkdownFile(md) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (tok.type === 'heading' && tok.depth >= 1 && tok.depth <= 3) {
      flushContent();
      if (tok.depth === 1 && root !== null) {
        throw new Error('Multiple H1 headings found — each file must have exactly one H1.');
      }
      ...
      if (stack.length) {
        ...
        if (parent.children?.some(c => c.id === node.id)) {
          throw new Error(`Duplicate node id "${node.id}" — rename one heading so sibling titles differ.`);
        }
